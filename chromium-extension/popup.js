/* IDM No Ads popup — polls the aria2 JSON-RPC endpoint directly on localhost.
 * The RPC port + secret are configured on the options page (the desktop app
 * shows the secret under Settings → Advanced). */
...
async function tick() {
  if (!cfg.rpcSecret) {
    ...
    if (go) go.onclick = () => chrome.runtime.openOptionsPage();
    return;
  }
  try {
...
// If we don't have the secret yet, ask the background to fetch it from the app.
chrome.runtime.sendMessage({ type: "ensure-config" });
// Re-render immediately when the background stores the freshly-fetched config.
